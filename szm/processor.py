...
class Processor:
    def __init__(self, memory, stack, out_screen, out_transcript, out_memory, out_command, in_keyboard, in_file, debug = False):
        ...
        self.lut_1op = {0x0: self.execute_1op_jz, 0x1: self.execute_1op_get_sibling, 0x2: self.execute_1op_get_child, 0x3: self.execute_1op_get_parent, 0x4: self.execute_1op_get_prop_len, 0x5: self.execute_1op_inc, 0x6: self.execute_1op_dec, 0x7: self.execute_1op_print_addr, 0x8: self.execute_1op_call_1s, 0x9: self.execute_1op_remove_obj, 0xA: self.execute_1op_print_obj, 0xB: self.execute_1op_ret, 0xC: self.execute_1op_jump, 0xD: self.execute_1op_print_paddr, 0xE: self.execute_1op_load, 0xF: self.execute_1op_not_or_call_1n}
        
        self.lut_0op = {0x0: self.execute_0op_rtrue, 0x1: self.execute_0op_rfalse, 0x2: self.execute_0op_print, 0x3: self.execute_0op_print_ret, 0x4: self.execute_0op_nop, 0x5: self.execute_0op_save, 0x6: self.execute_0op_restore, 0x7: self.execute_0op_restart, 0x8: self.execute_0op_ret_popped, 0x9: self.execute_0op_pop_or_catch, 0xA: self.execute_0op_quit, 0xB: self.execute_0op_new_line, 0xC: self.execute_0op_show_status, 0xD: self.execute_0op_verify, 0xE: self.execute_0op_extended, 0xF: self.execute_0op_piracy}
        
        self.lut_var = {0x0: self.execute_var_call_vs, 0x1: self.execute_var_storew, 0x2: self.execute_var_storeb, 0x3: self.execute_var_put_prop, 0x4: self.execute_var_sread_or_aread, 0x5: self.execute_var_print_char, 0x6: self.execute_var_print_num, 0x7: self.execute_var_random, 0x8: self.execute_var_push, 0x9: self.execute_var_pull, 0xA: self.execute_var_split_window, 0xB: self.execute_var_set_window, 0xC: self.execute_var_call_vs2, 0xD: self.execute_var_erase_window, 0xE: self.execute_var_erase_line, 0xF: self.execute_var_set_cursor, 0x10: self.execute_var_get_cursor, 0x11: self.execute_var_set_text_style, 0x12: self.execute_var_buffer_mode, 0x13: self.execute_var_output_stream, 0x14: self.execute_var_input_stream, 0x15: self.execute_var_sound_effect, 0x16: self.execute_var_read_char, 0x17: self.execute_var_scan_table, 0x18: self.execute_var_not, 0x19: self.execute_var_call_vn, 0x1A: self.execute_var_call_vn2, 0x1B: self.execute_var_tokenise, 0x1C: self.execute_var_encode_text, 0x1D: self.execute_var_copy_table, 0x1E: self.execute_var_print_table, 0x1F: self.execute_var_check_arg_count}
        
        self.lut_ext = {0x0: self.execute_ext_save, 0x1: self.execute_ext_restore, 0x2: self.execute_ext_log_shift, 0x3: self.execute_ext_art_shift, 0x4: self.execute_ext_set_font, 0x9: self.execute_ext_save_undo, 0xA: self.execute_ext_restore_undo, 0xB: self.execute_ext_print_unicode, 0xC: self.execute_ext_check_unicode}
    ...
    def execute_0op_extended(self, head, operands):
        tail = zoperator.Tail(head, False, False, False)
