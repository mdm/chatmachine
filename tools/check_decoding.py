            ...
            if (last_op[0] == 'J') or (last_op in ['CHECK_ARG_COUNT', 'GET_CHILD', 'TEST_ATTR']):
                ...
                if (ref_line == last_line + 1):
                    if (trail_addr == addr[:-1]):
                        trail.pop(0)
                        print 'Jump from:', last_line, asmlines[last_line],
                        print 'Jump to:', ref_line, asmlines[ref_line],
                else:
                    trail_addr = trail[0].split()[-1]
                ...
                if not (trail_addr == addr[:-1]):
                    ...
                    trail.pop(0)
                    print 'Call from:', last_line, asmlines[last_line],
                    print 'Call to:', ref_line, asmlines[ref_line],
                ...
                if not (trail_addr == addr[:-1]):
                    ...
                    trail.pop(0)
                    print 'Return from:', last_line, asmlines[last_line],
                    print 'Return to:', ref_line, asmlines[ref_line],
        last_op = operator
        last_line = ref_line
    #cmd = parts[3].strip()
    #print addr, cmd
