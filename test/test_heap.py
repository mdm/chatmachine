...
class TestHeap(unittest.TestCase):
    def setUp(self):
        self.bronze = szm.memory.Memory('data/Bronze.z5')
        self.curses = szm.memory.Memory('data/curses.z5')
        self.zork1 = szm.memory.Memory('data/zork1.z5')
    ...
    def test_init(self):
        self.assertTrue(self.compare_contents(self.bronze.data, ctypes.create_string_buffer(open('data/Bronze.z8').read())))
        self.assertTrue(self.compare_contents(self.curses.data, ctypes.create_string_buffer(open('data/curses.z5').read())))
        self.assertTrue(self.compare_contents(self.zork1.data, ctypes.create_string_buffer(open('data/zork1.z5').read())))

        self.assertFalse(self.compare_contents(self.bronze.data, ctypes.create_string_buffer(open('data/zork1.z5').read())))
